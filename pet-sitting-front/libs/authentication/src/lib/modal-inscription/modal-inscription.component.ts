...
import { UserModel , RoleEnum, NavBarService, UtilService} from '@pet-sitting-front/services';
...
export class ModalInscriptionComponent {
  ...
  constructor(public bsModalRef: BsModalRef, private userService: UserService,  private  navBarService: NavBarService,
  ...
  this.user.password = this.utilService.aesEncrypt(this.user.password  ?? '');
    this.userService.createUser(this.user) .subscribe({
      ...
      next: (data) => { 
        ...
        if(this.userConnected != null && this.userConnected.id!= null  && this.userConnected != undefined)
